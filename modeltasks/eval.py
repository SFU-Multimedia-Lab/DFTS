...
class CFeval(object):
	...
	def __init__(self, metrics, reshapeDims, dataGen):
...
class ODeval(object):
	...
	def __init__(self, metrics, reshapeDims, dataGen):
		...
		self.n_classes   = dataGen.classes
		...
		self.prediction_results = [list() for _ in range(self.n_classes + 1)]*len(self.iou)
		...
		self.finalResults = dict(zip(iou, self.prediction_results))
		self.groundTruth = []
		self.imageId     = []

	def evaluate(self, remoteOut, groundTruth, imageId):
		...
		for i in self.iou:
			self.predictOnBatch(i, remoteOut, imageId, self.reshapeDims[0], self.reshapeDims[1], decoding_iou_threshold=i)

	def simRes(self):
		userRes = {}
		num_gt_per_class = self.get_num_gt_per_class()
		for i in self.finalResults.keys():
			userRes[i] = iterateOverIOU(self.finalResults[i], i, num_gt_per_class)
		return userRes

	def iterateOverIOU(preds, iou, num_gt_per_class):
	...
	def predictOnBatch(self, iou, remoteOut, imageId, imgH, imgW, decoding_confidence_thresh=0.01, decoding_iou_threshold=0.45):
		...
		for k, batch_item in enumerate(remoteOut):
			...
			for box in batch_item:
				...
				self.self.finalResults[iou][class_id].append(prediction)
